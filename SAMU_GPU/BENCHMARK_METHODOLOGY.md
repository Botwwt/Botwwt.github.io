...
每个完整模型都包含相同的词嵌入、归一化、双输入分支、因果逐通道卷积、乘法连接、输出投影和门控前馈网络，唯一切换项是 SAMU 或 RG‑LRU 混合器。质量轨道使用六层小模型；系统速度轨道直接使用表 2 的 12 层 400M 与 24 层 1.3B 配置。

- 训练质量：enwik8 原始字节流，按前 90%/中间 5%/最后 5% 固定切分；L=256，B=32，每步 8192 字节；随机种子 17、29、43；每个种子 3000 次更新。
- 长度缩放：L=2048、4096、8192，对应 B=4、2、1，总词元固定为 8192。
- 模型缩放一：小、中、大三套单卡配置，用于定位递推宽度拐点。
- 模型缩放二：直接使用 Griffin 表 2 的宽度、递推宽度与层数以及 32K 词表；400M 和 1.3B 点真实运行，7B 在分配前计算 FP32 参数、梯度与 Adam 两组动量的最低容量。
...
- 1.3B 推理是完整系统速度实验，但随机权重不能提供语言质量结论；
- enwik8 小模型质量差异不能直接外推为 1.3B 困惑度或下游任务差异。

7B 与多卡问题需要张量并行、ZeRO、同规模训练检查点和相同大规模语料预算。当前结果支持单张 H800 上公开的完整优化器步骤、1.3B 配置推理、enwik8 三随机种子质量和递推内核分解。
