...
export const lessons = [
  ...
  },
  {
    ...
    body: `
      ...
      <p>这一思想不是 SAMU 发明的。RecurrentGemma 已经在 TPU 上使用优化过的 Pallas 顺序扫描；本项目做的是 NVIDIA GPU 上的 Triton 对应实现，并给 SAMU 使用同等级路径。</p>`,
  },
  {
  ...
  },
  {
    ...
    body: `
      <p>Griffin 的 RG‑LRU 方程先用两组块对角矩阵生成输入门和递推门。论文在模型并行部分使用 16 个门分块；本报告主对比也固定为 16。每个实状态通道都有自己的两个动态门。</p>
      ${math(String.raw`\begin{aligned}
  ...
  },
  {
    ...
    body: `
      ...
      <p>完整 Griffin 递推块的双方都具有三张共同矩阵：输入分支 <code>linear_x</code>、门控分支 <code>linear_y</code> 和输出矩阵 <code>linear_out</code>。SAMU 直接使用 <code>linear_x</code> 的输出作为写入，只额外添加两个归一化控制方向和每模态的静态谱参数；RG‑LRU 则在 <code>linear_x</code> 之后添加两张块对角门矩阵。</p>
      ${math(String.raw`\begin{aligned}
      ...
      <div class="term-note"><b>这项修改怎样进入系统对比</b><p>写入直接复用外层输入分支，因此参数量、训练驻留显存和每步矩阵乘法都会随结构改变。系统实验同时报告双方实际参数量、峰值已分配显存和完整步骤耗时，不把孤立递推内核的优势外推为整模型优势。</p></div>`,
  },
  {
  ...
  },
  {
    ...
    body: `
      <p>RG‑LRU 的隐藏状态是实数；SAMU 的隐藏状态是复数。公平比较不能把一个复数当成一个实数：这里用 <i>D</i> 个 RG‑LRU 实状态，对比 <i>D/2</i> 个 SAMU 复数模态，因此双方都是 <i>D</i> 个 FP32 实标量，递推状态占用的显存完全相同。</p>
      ${math(String.raw`\begin{aligned}
      \text{RG-LRU:}\quad h_t&=a_t\odot h_{t-1}+\sqrt{1-a_t^2}\odot(i_t\odot x_t),\\
      \text{SAMU:}\quad z_{j,t}&=\rho_{j,t}e^{\mathrm{i}\phi_{j,t}}z_{j,t-1}+w_{j,t}.
      \end{aligned}`, "RG‑LRU 每个通道更新一个实数；SAMU 每个模态同时更新实部和虚部。SAMU 的单模态计算更重，优势不来自更便宜的乘加。")}
      <div class="profile-table-wrap"><table class="profile-table architecture-table"><thead><tr><th>比较项目</th><th>RG‑LRU</th><th>SAMU</th></tr></thead><tbody>
        <tr><td>状态表示</td><td><i>D</i> 个 FP32 实状态，每个通道只有一个实数。</td><td><i>D/2</i> 个复数模态，每个模态包含实部和虚部，共计 <i>D</i> 个 FP32 实标量。</td></tr>
        <tr><td>每个状态的更新代价</td><td>实数衰减、实数写入和加法，单通道运算较便宜。</td><td>衰减后还要做二维旋转，需要指数、正弦、余弦和复数乘加；单模态运算更贵。</td></tr>
        <tr><td>每个词元产生的动态控制</td><td>输入门和保留门都为每个实状态通道单独取值，两个门合计为 <code>[B,L,2D]</code>。</td><td>每个词元只产生一个径向控制量 <i>c</i> 和一个相位控制量 <i>d</i>，合计为 <code>[B,L,2]</code>；内核再结合每个模态的静态参数重建转移。</td></tr>
        <tr><td>控制投影随宽度的增长</td><td>官方 16 组块对角门仍要生成两组通道级控制；状态越宽，门参数、矩阵乘法和反向保存量越大。</td><td>控制投影始终只输出两个数；状态变宽时增加的是静态模态参数和必要的状态计算，不增加逐通道动态门。</td></tr>
        <tr><td>短序列路径</td><td>顺序扫描把状态留在寄存器中。</td><td>同样使用顺序扫描；避免分块启动与摘要开销。</td></tr>
        <tr><td>长序列路径</td><td>使用精确分块扫描；本轮 H800 调优选中每块 32 个词元。</td><td>使用同一种精确分块思路；本轮 H800 调优选中每块 16 个词元，并在扫描内由两个共享控制量重建各模态的旋转。</td></tr>
        <tr><td>单步解码</td><td>低批量融合两组块对角门与状态更新；高批量把两组门合并为一次分组矩阵乘，使用 Tensor Core。</td><td>在融合、预打包静态谱和不同模态分块之间按批量实测选择。</td></tr>
      </tbody></table></div>
      <div class="term-note"><b>16 和 32 不是方法优劣</b><p>它们只是双方在当前 H800、当前形状下分别测出的最快分块长度。“精确”表示分块前后仍计算同一个递推方程，没有近似或跳过词元。分块越小，并行块更多但摘要开销更大；分块越大，块内顺序依赖更长。两边的最佳值不同很正常，不能把 16 与 32 本身写成 SAMU 的贡献。</p></div>
      ${math(String.raw`\begin{aligned}
      \text{RG-LRU 动态门输出}&:\ [B,L,2D_{\mathrm{RNN}}],\\
      \text{SAMU 动态控制}&:\ [B,L,2].
      \end{aligned}`, "这里比较随词元变化、为状态转移服务的动态描述。输入、状态输出等双方都必须处理的数据没有从口径中删除。")}
      <p><b>SAMU 真正省掉的是通道级动态控制，而不是状态。</b>在训练中，这会减少门投影的参数与矩阵乘法、需要在前向中产生的动态值，以及反向传播需要读取或重算的门信息；这正是 400M 配置快 1.26–1.29 倍、1.3B 配置快 1.24–1.26 倍的主要来源。代价是扫描内对每个复数模态重建指数和旋转。</p>
      <p>因此优势有明确边界：状态宽、序列长、需要反向传播时，省下的门投影与数据移动更容易超过额外的旋转计算；极小宽度或单步解码时，RG‑LRU 的实数更新很便宜，而完整模型的公共矩阵乘法又占据大部分时间，SAMU 不会自然获得同样大的加速。</p>`,
  },
  {
    title: "训练实验：从扫描内核到表 2 完整模型",
    context: ["自定义反向", "400M / 1.3B", "固定每步词元数"],
    lead: "训练侧沿用 Griffin 第 4 节和附录 D 的层级：扫描后端、完整模型后端消融、固定词元的长序列训练步。",
    body: `
      ...
      <div><b>图 8(b) 后端消融</b><p>在 400M 和 1.3B 完整 Hawk 模型中分别装入 PyTorch 逐步参考、片上顺序扫描、BF16/FP32 结合扫描和 16/32 步精确分块扫描，只改变递推扫描后端。</p></div>
      <div><b>图 3 长度与规模轴</b><p>采用 Griffin 表 2 的 400M、1.3B、7B 宽度、递推宽度和层数。400M 与 1.3B 真实运行；7B 在分配前计算同精度 AdamW 的最低驻留容量，因为该下界已经超过 80 GiB。</p></div></div>
      <p>完整训练步固定处理 8192 个词元，序列长度取 2K、4K、8K，批量相应为 4、2、1；计时包含完整模型前向、交叉熵、反向传播、梯度裁剪和 AdamW 更新。随机词元只提供固定形状的算子负载，不用于损失或准确率结论。</p>
      ${steps([
        ...
        {title: "保留单卡容量边界", text: "7B 不改用低精度优化器、CPU 卸载或缩短模型来制造可运行数字；只报告 FP32 参数、梯度和 Adam 两组动量的最低字节数。", check: "容量下界与真实计时分开显示，不能从 OOM 推测速度。"},
      ])}
  ...
  },
  {
    ...
    lead: "推理主结果采用 Griffin 表 2 的 1.3B 系统形状，包括 24 个完整 Hawk 递推块、32K 词表输出和下一个词元选择。随机权重只用于固定算子形状的速度测量。",
    body: `
  ...
  {
    title: "如何证明 GPU 适配真正有效",
    context: ["方程", "速度", "端到端"],
    lead: "系统结论必须同时经过方程正确性、同等级内核和完整模型三道门槛。",
    body: `
      <ol class="research-checklist">
        ...
        <li><b>边界完整：</b>训练同时报告 400M、1.3B 和 7B 单卡容量结果；推理同时报告 B=16 延迟、空提示/4K 提示、共同批量搜索、完整轨迹吞吐和缓存字节。</li>
      </ol>
      <p>系统侧同时报告递推前向、训练反向、扫描后端消融、400M/1.3B 完整优化器步骤、7B 单卡容量边界和 1.3B 完整生成轨迹。共享控制的 GPU 适配优势只从这些实测系统指标得出，不延伸到准确率、损失或模型能力。</p>`,
  },
];
