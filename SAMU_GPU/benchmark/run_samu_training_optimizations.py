"""A/B exact SAMU training paths on the Griffin length/scale matrix."""
...
def main() -> None:
    ...
    for index, (scale, dimensions) in enumerate(SCALES):
        ...
        for backend in order:
            ...
            model, _ = build_model(config, "samu", args.seed)
            model = model.cuda()
    ...
    atomic_json(args.output, {
        "schema_version": 1,
        "comparison": (
            "exact factorized state-stationary serial scan versus exact "
            "differentiable 16- and 32-token affine chunk scans"
        ),
        "rows": rows,
    })
